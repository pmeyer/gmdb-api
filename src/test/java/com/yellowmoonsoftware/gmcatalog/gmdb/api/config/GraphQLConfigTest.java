...
class GraphQLConfigTest {
    ...
    void configureRegistersExpectedScalars() {
        final GraphQLConfig config = new GraphQLConfig();
        final RuntimeWiring.Builder builder = mock(RuntimeWiring.Builder.class);
        ...
        final RuntimeWiringConfigurer configurer = config.configure();
        configurer.configure(builder);

        final var inOrder = inOrder(builder);
